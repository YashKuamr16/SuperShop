...
def homePage(Request):
    # ye limit use kerne ke liye hota hai meri pass 10 product hai lakin mane screen per 8 products dikhae hai ager main isme se order.by hata dunga tho mujhe meri project me all product dikhi denje
...
def shopPage(Request, mc, sc, br):
    if (mc == 'All' and sc == 'All' and br == 'All'):
        ...
        products = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc), subcategory=Subcategory.objects.get(
            name=sc), brand=Brand.objects.get(name=br)).order_by("-id").order_by("-id")
...
def searchPage(Request):
    if (Request.method == "POST"):
        ...
        try:
            ...
            brand = None
        products = Product.objects.filter(Q(name__icontains=search) | Q(maincategory=maincategory) | Q(
            subcategory=subcategory) | Q(brand=brand) | Q(color=search) | Q(description__icontains=search))
        maincategory = Maincategory.objects.all().order_by("-id")
        subcategory = Subcategory.objects.all().order_by("-id")
        brand = Brand.objects.all().order_by("-id")
...
def signupPage(Request):
    if (Request.method == "POST"):
        ...
        if (password == cpassword):
            ...
            error(Request, "Password and ConfirmPassword Doesn't Matched!!")
...
def forgetPassword3Page(Request):
    ...
    if (Request.method == "POST"):
        ...
        if (password == cpassword):
            ...
            error(Request, "Password and Confirm Passqord Does't Matched.... ")
    if (username):
